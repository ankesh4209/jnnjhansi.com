...
$PAGE_CONTENTS	= ReadTemplate("../$TEMPLATE_DIR/admin/disposed_proposal.html");
...
function viewcomplainant($db,$search,$sdate,$edate,$category)
 {
   ...
   $S1	= $S2 = ReadTemplate("../$TEMPLATE_DIR/admin/prop_complainantGrid.html");
  ...
  if($edate!="")
  ...
  else
   {
   ...
   else
    {
	 ...
	 $count="select count(c_id) as total from complainant where status='1' and ComplainType='1' $sdate1 $edate1 $category1 $search_key";
   $db->query($count);
	 $row = $db->fetch_assoc();
   $TOTAL_RECORDSET = $row['total'];
  
   $query="select * from complainant where status='1' and ComplainType='1' $sdate1 $edate1 $category1 $search_key limit  $page, $MAX";
