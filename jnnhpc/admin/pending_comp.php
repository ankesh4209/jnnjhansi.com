...
$PAGE_CONTENTS	= ReadTemplate("../$TEMPLATE_DIR/admin/pending_comp.html");
...
function viewcomplainant($db,$search,$sdate,$edate,$category)
 {
   ...
   $S1	= $S2 = ReadTemplate("../$TEMPLATE_DIR/admin/pend_complainantGrid.html");
  ...
  if($edate!="")
  ...
  else
   {
   ...
   else
    {
	 ...
	 $count="select count(c_id) as total from complainant where tdate>='$today' and status='0' and ComplainType='0' $sdate1 $edate1 $category1 $search_key";
   $db->query($count);
	 $row = $db->fetch_assoc();
   $TOTAL_RECORDSET = $row['total'];
  
   $query="select * from complainant where tdate>='$today' and status='0' and ComplainType='0' $sdate1 $edate1 $category1 $search_key order by c_id desc limit  $page, $MAX";
   ...
   $db->query($query);
		if($db->num_rows())
		{
		 ...
		 if($page > 0)
			{	
        $prevpage=$page - $MAX;
				$PREV_PAGE_LINK="<<a href='pending_comp.php?page=$prevpage&max=$MAX&$next_links' >Prev</a>";
			}
			
			if($TOTAL_RECORDSET > $lastrow)
			{	
        $NEXT_PAGE_LINK="<a href='pending_comp.php?page=$lastrow&max=$MAX&$next_links' >Next></a>";
			}
       ...
       if ($lastrow-$i==$MAX)
          ...
          $PAGE_NAVS.=" <B>".$toPrint."</b> | ";
				...
				{	
          $PAGE_NAVS.=" <a href='pending_comp.php?page=$i&max=$MAX&left_id=1&$next_links' >$toPrint</a> |";
				}
				$TOTAL_PAGES=$toPrint;
			}
		...
		{
      $NEWS_LIST="<tr><td colspan='7' height='25' valign='middle' align='center'>No Pending Complainant Records Found</td></tr>";
    }
