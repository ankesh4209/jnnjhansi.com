...
$PAGE_CONTENTS	= ReadTemplate("../$TEMPLATE_DIR/admin/officer.html");
...
function viewofficer($db)
 {
   global $S1,$S2,$slno,$page,$prevpage,$NEXT_PAGE_LINK,$TOTAL_PAGES,$PREV_PAGE_LINK,$title,$nid,$cdate,$TEMPLATE_DIR,$NEWS_LIST;
   global $TOTAL_RECORDSET,$PAGE_NAVS,$status,$designation,$fid,$fname,$sfdesign,$sfpay,$sfadsalary,$faddress,$fcontact,$row_color;
   
   $S1	= $S2 = ReadTemplate("../$TEMPLATE_DIR/admin/officerGrid.html");
	 ...
	 $count="select count(o_id) as total from officer";
   $db->query($count);
	 $row = $db->fetch_assoc();
   $TOTAL_RECORDSET = $row['total'];
  
   $query="select * from officer limit  $page, $MAX";
   ...
   $db->query($query);
		if($db->num_rows())
		{
		 ...
		 if($page > 0)
			{	$prevpage=$page - $MAX;
				$PREV_PAGE_LINK="<<a href='officer.php?page=$prevpage&max=$MAX&$next_links' >Prev</a>";
			}
			
			if($TOTAL_RECORDSET > $lastrow)
			{	$NEXT_PAGE_LINK="<a href='officer.php?page=$lastrow&max=$MAX&$next_links' >Next></a>";
			}
       ...
       if ($lastrow-$i==$MAX)
				...
				{	
          $PAGE_NAVS.=" <a href='officer.php?page=$i&max=$MAX&left_id=1&$next_links' >$toPrint</a> |";
				}
				$TOTAL_PAGES=$toPrint;
			}
		...
		{
      $NEWS_LIST="<tr><td colspan='5'>No officer Records Found</td></tr>";
    }
...
function deleteofficer($fid, $db)
 {	
	global $PROMPT;
 	$officer = implode("," ,$fid);
 	$delete = "delete from officer where o_id in ($officer)";
	$db->query($delete);
