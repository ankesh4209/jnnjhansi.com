...
if($_POST['submit']!="")
{
 viewcomplainant($db,$sdate,$edate,$category,$type);
}
...
$PAGE_CONTENTS	= ReadTemplate("../$TEMPLATE_DIR/admin/reports.html");
...
function viewcomplainant($db,$sdate,$edate,$category,$type)
 {
   ...
   $S1	= $S2 = ReadTemplate("../$TEMPLATE_DIR/admin/reportsGrid.html");
  ...
  if($edate!="")
   ...
   elseif($type==4)
   ...
   else
    {
	 ...
	 $count="select count(c_id) as total from complainant where 1=1 $edate1 $sdate1 $category1 $search_type";
   $db->query($count);
	 $row = $db->fetch_assoc();
   $TOTAL_RECORDSET = $row['total'];
  
   $query="select * from complainant where 1=1 $edate1 $sdate1 $category1 $search_type limit  $page, $MAX";
   ...
   $db->query($query);
		if($db->num_rows())
		{
		 ...
		 if($page > 0)
			{	
        $prevpage=$page - $MAX;
				$PREV_PAGE_LINK="<<a href='complainant.php?page=$prevpage&max=$MAX&$next_links' >Prev</a>";
			}
			
			if($TOTAL_RECORDSET > $lastrow)
			{	
        $NEXT_PAGE_LINK="<a href='complainant.php?page=$lastrow&max=$MAX&$next_links' >Next></a>";
			}
       ...
       if ($lastrow-$i==$MAX)
				...
				{	
          $PAGE_NAVS.=" <a href='complainant.php?page=$i&max=$MAX&left_id=1&$next_links' >$toPrint</a> |";
				}
				$TOTAL_PAGES=$toPrint;
			}
		...
		{
      $NEWS_LIST="<tr><td colspan='8' height='25' valign='middle' align='center'>No Complainant Records Found</td></tr>";
    }
