...
$PAGE_CONTENTS	= ReadTemplate("../$TEMPLATE_DIR/admin/ward_wise_proposals.html");
...
function viewcomplainant($db,$db1,$sdate,$edate,$category,$WardId,$cmode)
 { 
  ...
  if($edate!="")
   ...
   $query="select * from complainant where 1=1 and status =0  $WardId1 and ComplainType='1' $sdate1 $edate1 $category1 $cmode";
	      ...
	      $wardno=stripslashes($rows['WardId']);
			 ...
			 // echo $_POST['cmode'];
				  ...
				  if($_POST['cmode']==' ik’kZnx.k'){
				                ...
				                $corpname="<th align='center'>ik’kZn</th>";
				                $ward="<th align='center'><span style='font-family:Kruti Dev 011;font-size:14px'>Ward No</th>";
			...
			$complain_list.="<tr><td colspan='7' align='right'><b><a href='javascript: void(0)' onclick='window.open(\"print_proposals.php?nature=$p_category&sdate=$p_sdate&edate=$p_edate&WardId=$p_WardId&cmode=$p_mode\",\"windowname1\",\"width=650, height=550,scrollbars=yes\")'><input type='button' name='click' value='Print' class='button'></a></b></td></tr>";
		}
...
function viewAllcomplainant($db,$db1,$sdate,$edate,$category,$WardId,$cmode)
 {
  ...
  if($edate!="")
   ...
   $date_diff=$date_diff/(24*60*60);
	  ...
	  $query="select * from complainant where 1=1 and status =0  $WardId1 and ComplainType='1' $sdate1 $edate1 $c_mode";
