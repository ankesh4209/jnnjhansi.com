...
function viewcomplainant($db,$db1,$sdate,$edate,$category,$status,$cmode)
 { 
  ...
  if($edate!="")
   ...
   $query="select * from complainant where 1=1 $status1 and ComplainType='0' $sdate1 $edate1 $category1 $cmode";
   ...
   $db->query($query);
		if($db->num_rows())
		{
	      ...
	      $wardno=stripslashes($rows['WardId']);
			...
			$complain_list.="<tr><td colspan='7' align='right'><b><a href='javascript: void(0)' onclick='window.open(\"print_dep_reports.php?nature=$p_category&sdate=$p_sdate&edate=$p_edate&status=$p_status&cmode=$p_mode\",\"windowname1\",\"width=650, height=550,scrollbars=yes\")'><input type='button' name='click' value='Print' class='button'></a></b></td></tr>";
		}
...
function viewAllcomplainant($db,$db1,$sdate,$edate,$category,$status,$cmode)
 {
  ...
  if($edate!="")
   ...
   $date_diff=$date_diff/(24*60*60);
	  ...
	  $query="select * from complainant where 1=1 $status1 and ComplainType='0' $sdate1 $edate1 $c_mode";
