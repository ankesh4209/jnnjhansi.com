<?php	
include("jnndiwas/config/data.config.php");
include("jnndiwas/phplib/functions.library.php");
include("jnndiwas/phplib/class.database.php");
include("jnndiwas/phplib/data.constant.php");
...
if($_POST['submit']!="")
{
 ...
 $PAGE_CONTENTS	= ReadTemplate("$TEMPLATE_DIR/searchResult_jnndiwas.html");
}
else
{
 $PAGE_CONTENTS	= ReadTemplate("$TEMPLATE_DIR/search_jnndiwas.html");
}
...
function Search_comp($db,$complainno,$cdate,$contactno,$cname)
 {
   ...
   if($db->num_rows())
   {
     ...
     $complain_detail.="<tr>
                        ...
                        <td>$complain_date</td>
                        <td>$status</td>
                        <td><input type='button' name='print' value='View' onclick=\" openPopup('jnndiwas_print.php?id=$cid');\"></td>
                       </tr>";
     }
   }
   else
