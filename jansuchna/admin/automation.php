...
function viewpage($db,$rid)
 {
   ...
   if($rid!="")
   $query="select * from tbl_automation where a_rno='$rid' order by a_id desc limit  $page, $MAX ";
   else
...
function deleteaboutus($aid, $db)
 {	
	global $PROMPT;
   	...
   	$product = implode(",", $aid);
    $sel_reg_id="select a_rno from tbl_automation where a_id in ($product)"; 
    $db->query($sel_reg_id);
   
   while($rows = $db->fetch_array())
   {
		
 	$delete1 = "delete from tbl_question where reg_id='".$rows['a_rno']."'";
	$db->query($delete1);
	
    $delete = "delete from tbl_automation where a_rno='".$rows['a_rno']."'";
    $db->query($delete);
