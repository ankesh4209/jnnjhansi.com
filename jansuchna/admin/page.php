...
function ViewDepartment($db)
 {
   ...
   global $TOTAL_RECORDSET,$PAGE_NAVS,$status,$did,$dname,$off_name,$off_contact,$off_dsg;
   ...
   $S1	= $S2 = ReadTemplate("../$TEMPLATE_DIR/admin/pageDisplayGrid.html");
	...
	$MAX=10;
	 ...
	 $count="select count(d_id) as total from tbl_department";
   $db->query($count);
	 $row = $db->fetch_assoc();
   $TOTAL_RECORDSET = $row['total'];
  
   $query="select * from tbl_department limit  $page, $MAX";
		...
		if($db->num_rows())
		{
		  $slno=$page+1;
			while($rows = $db->fetch_array())
			{
			  ...
			  $off_contact =$rows['d_off_contact'];
			  $off_dsg =stripslashes($rows['d_off_designation']);
...
function deleteaboutus($did, $db)
 {	
	global $PROMPT;

	$product = implode(",", $did);

 	$delete = "delete from tbl_department where d_id in ($product)";
	$db->query($delete);
