...
$PAGE_CONTENTS	= ReadTemplate("../$TEMPLATE_DIR/admin/complete.html");
...
function viewpage($db,$rid)
 {
   ...
   $S1	= $S2 = ReadTemplate("../$TEMPLATE_DIR/admin/completeDisplayGrid.html");
	...
	$MAX=10;
	 ...
	 if($rid!="")
	  $count="select count(a_id) as total from tbl_automation where a_rno='$rid' and app_status='2'";
	 else
	  $count="select count(a_id) as total from tbl_automation where app_status='2'";
   ...
   if($rid!="")
   $query="select * from tbl_automation where a_rno='$rid' and app_status='2' order by a_id desc limit  $page, $MAX ";
   else
   $query="select * from tbl_automation where app_status='2' order by a_id desc limit  $page, $MAX ";
   ...
   $db->query($query);
        ...
        elseif($status==2)
		 ...
		 if($page > 0)
			{	$prevpage=$page - $MAX;
				$PREV_PAGE_LINK="<<a href='complete_form.php?page=$prevpage&max=$MAX&$next_links' >Prev</a>";
			}
			
			if($TOTAL_RECORDSET > $lastrow)
			{	$NEXT_PAGE_LINK="<a href='complete_form.php?page=$lastrow&max=$MAX&$next_links' >Next></a>";
			}
       ...
       if ($lastrow-$i==$MAX)
          ...
          $PAGE_NAVS.=" <B>".$toPrint."</b> | ";
				...
				{	
          $PAGE_NAVS.=" <a href='complete_form.php?page=$i&max=$MAX&left_id=1&$next_links' >$toPrint</a> |";
				}
				$TOTAL_PAGES=$toPrint;
			}
