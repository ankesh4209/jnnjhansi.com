...
$PAGE_CONTENTS	= ReadTemplate("$TEMPLATE_DIR/finance_docs.html");
...
function ViewTenders($db)
 {
   ...
   $S1	= $S2 = ReadTemplate("$TEMPLATE_DIR/notice_infoGrid.html");
   ...
   $db->query($query);
		if($db->num_rows())
		{
		 ...
		 if($page > 0)
			{	$prevpage=$page - $MAX;
				$PREV_PAGE_LINK="<<a href='finance_docs.php?page=$prevpage&max=$MAX&$next_links' >Prev</a>";
			}
			
			if($TOTAL_RECORDSET > $lastrow)
			{	$NEXT_PAGE_LINK="<a href='finance_docs.php?page=$lastrow&max=$MAX&$next_links' >Next></a>";
			}
       ...
       if ($lastrow-$i==$MAX)
          ...
          $PAGE_NAVS.=" <B>".$toPrint."</b> | ";
				...
				{	
          $PAGE_NAVS.=" <a href='finance_docs.php?page=$i&max=$MAX&left_id=1&$next_links' >$toPrint</a> |";
				}
				$TOTAL_PAGES=$toPrint;
			}
		$pages="Pages:";
