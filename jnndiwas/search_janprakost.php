...
if($_POST['submit']!="")
{
 ...
 $PAGE_CONTENTS	= ReadTemplate("$TEMPLATE_DIR/searchResult_janprakost.html");
}
else
{
 $PAGE_CONTENTS	= ReadTemplate("$TEMPLATE_DIR/search_janprakost.html");
}
...
ReplaceContent(Array("LEFTBAR","TOPBAR", "BOTTOMBAR", "PAGE_CONTENTS", "TEMPLATE"));
print $TEMPLATE;
flush();
...
function Search_comp($db,$complainno,$cdate,$contactno,$cname)
 {
   ...
   if($db->num_rows())
   {
     ...
     $complain_detail.="<tr>
                        ...
                        <td>$complain_date</td>
                        <td>$status</td>
                        <td><input type='button' name='print' value='View' onclick=\" openPopup('janprakost_print.php?id=$cid');\"></td>
                       </tr>";
     }
   }
   else
