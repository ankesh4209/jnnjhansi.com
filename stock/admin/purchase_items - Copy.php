...
function viewpage($db,$db1,$rid)
 {
   ...
   global $TOTAL_RECORDSET,$PAGE_NAVS,$status,$ItemId,$ItemType,$ItemName,$Qty,$DeliverQty,$RemainQty,$TotalQty;
   ...
   $S1	= $S2 = ReadTemplate("../$TEMPLATE_DIR/admin/purchaseDisplayGrid.html");
	 ...
	 if($_GET['ItemTypeId']!=''){
	  ...
	  $count="SELECT count(t.total) as total FROM ( select count(1) as total from epr_stocks where 1 $sql group by ItemId) as t";
   ...
   //$query="SELECT i.ItemId as ItemId,i.ItemName as ItemName,s.Qty as Qty,i.ItemType as ItemType FROM epr_items as i INNER JOIN epr_stocks as s ON i.ItemId=s.ItemId where 1 $sql order by ItemId DESC limit  $page, $MAX ";
    $query="SELECT * from epr_stocks  where 1 $sql group by  ItemId limit  $page, $MAX ";
		...
		if($db->num_rows())
		{
		  $slno=$page+1;
			while($rows = $db->fetch_array())
			{
			  ...
			  $TotalQty=0;	
			  $query1="select Qty from epr_stocks where ItemId='".$ItemId."'";
              $res1=$db1->query($query1);
			  while($rows1 = $db1->fetch_array($res1)){
				    $TotalQty+=$rows1['Qty'].'<br>';
