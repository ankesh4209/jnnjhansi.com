...
$PAGE_CONTENTS	= ReadTemplate("../$TEMPLATE_DIR/admin/edit_firm.html");
...
function editNature($db,$cid)
 {
    ...
    $update="update epr_firms set firm_name='$cname' where firm_id='$cid'";
    $db->query($update);
 }
...
function GetNature($db,$id)
 {
   global $name;
   $sql="select firm_name from epr_firms where firm_id='$id'";
   $res=$db->query($sql);
   $rows=$db->fetch_array($res);
   $name=stripslashes($rows['firm_name']);
 }
?>