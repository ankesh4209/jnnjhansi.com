...
viewpage($db,$db1,$db2);
GetDepartments($db,$_GET['DepartmentId']);
 
$PAGE_CONTENTS	= ReadTemplate("../$TEMPLATE_DIR/admin/dept_wise_items.html");
...
function viewpage($db,$db1,$db2)
 {
   ...
   global $TOTAL_RECORDSET,$PAGE_NAVS,$status,$ItemId,$DistributionDate,$DistributionId,$Qty,$ItemName,$ReceiverName,$DepartmentName,$strItemName,$strQty,$sdate,$edate;
   
   $S1	= $S2 = ReadTemplate("../$TEMPLATE_DIR/admin/deptwiseDisplayGrid.html");
	...
	$MAX=10;
	 ...
	 if($_GET['DepartmentId']!=''){
	   $sql.=" and DepartmentId='".$_GET['DepartmentId']."'";
	   ...
	   $sql.=" and DistributionDate>='".$s_date1."' and DistributionDate<='".$e_date1."'";
	   }
	 
	  $count="select count(t.total) as total from ( SELECT count(1) as total from `epr_distributions` where 1 $sql group by DepartmentId,AddedDateTime ) as t";
	 
   $db->query($count);
	 $row = $db->fetch_assoc();
   $TOTAL_RECORDSET = $row['total'];
  
   $query="select * from epr_distributions where 1 $sql group by DepartmentId,AddedDateTime order by DistributionId ASC limit  $page, $MAX ";
   ...
   $db->query($query);
		if($db->num_rows())
		{
		 ...
		 if($page > 0)
			{	$prevpage=$page - $MAX;
				$PREV_PAGE_LINK="<<a href='dept_wise_items.php?page=$prevpage&max=$MAX&DepartmentId=".$_GET['DepartmentId']."' >Prev</a>";
			}
			
			if($TOTAL_RECORDSET > $lastrow)
			{	$NEXT_PAGE_LINK="<a href='dept_wise_items.php?page=$lastrow&max=$MAX&DepartmentId=".$_GET['DepartmentId']."' >Next></a>";
			}
       ...
       if ($lastrow-$i==$MAX)
          ...
          $PAGE_NAVS.=" <B>".$toPrint."</b> | ";
				...
				{	
          $PAGE_NAVS.=" <a href='dept_wise_items.php?page=$i&max=$MAX&left_id=1&$DepartmentId=".$_GET['DepartmentId']."' >$toPrint</a> |";
				}
				$TOTAL_PAGES=$toPrint;
			}
...
function GetItemName($db1,$ItemId)
 ...
 function GetDepartments($db,$DId)
 {
	global $departments;
 ...
 {
   //global $ItemName;
    $sql="select * from epr_department where nature_id='".$DepartmentId."'";
    $result=$db1->query($sql);
	$row=$db1->fetch_array();
    $DepartmentName=$row['nature_name'];
