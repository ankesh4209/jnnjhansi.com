...
function viewpage($db,$db1,$rid)
 {
   ...
   $query="SELECT i.ItemId as ItemId,i.ItemName as ItemName,sum(s.Qty) as Qty,i.ItemType as ItemType FROM epr_items as i INNER JOIN epr_stocks as s ON i.ItemId=s.ItemId where 1 $sql group by ItemId limit  $page, $MAX ";
...
function GetCategoryName($db1,$Cat_id)
 ...
 function GetCategory($db,$ItemTypeId)
 {
    ...
    $row=$db->query($sql);
      $category.="<select  name='ItemTypeId'   onchange='document.page1.submit();'>";
