...
$PAGE_CONTENTS	= ReadTemplate("../$TEMPLATE_DIR/admin/firms.html");
...
function viewNature($db)
 {
   ...
   $S1	= $S2 = ReadTemplate("../$TEMPLATE_DIR/admin/firmGrid.html");
	 ...
	 $count="select count(firm_id) as total from epr_firms";
   $db->query($count);
	 $row = $db->fetch_assoc();
   $TOTAL_RECORDSET = $row['total'];
  
   $query="select * from epr_firms order by firm_name limit  $page, $MAX";
   ...
   $db->query($query);
		if($db->num_rows())
		{
		  $slno=$page+1;
			while($rows = $db->fetch_array())
			{
			  $row_color = ($slno % 2) ? $color1 : $color2;
		   	$fid=$rows['firm_id'];
			  $cname=stripslashes($rows['firm_name']);
		 ...
		 if($page > 0)
			{	$prevpage=$page - $MAX;
				$PREV_PAGE_LINK="<<a href='firms.php?page=$prevpage&max=$MAX&$next_links' >Prev</a>";
			}
			
			if($TOTAL_RECORDSET > $lastrow)
			{	$NEXT_PAGE_LINK="<a href='firms.php?page=$lastrow&max=$MAX&$next_links' >Next></a>";
			}
       ...
       if ($lastrow-$i==$MAX)
				...
				{	
          $PAGE_NAVS.=" <a href='firms.php?page=$i&max=$MAX&$next_links' >$toPrint</a> |";
				}
				$TOTAL_PAGES=$toPrint;
			}
...
function deleteNature($fid,$db)
 {	
	 global $PROMPT;
     $fid = implode("," ,$fid);
 	$delete = "delete from epr_firms where firm_id in ($fid)";
	$db->query($delete);
	$total = $db->affected_rows();
	$PROMPT = "Total $total firms records have been deleted.";
 }
