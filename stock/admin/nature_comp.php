...
function viewNature($db)
 {
   ...
   $S1	= $S2 = ReadTemplate("../$TEMPLATE_DIR/admin/naturecompGrid.html");
	 ...
	 $count="select count(nature_id) as total from epr_department";
   $db->query($count);
	 $row = $db->fetch_assoc();
   $TOTAL_RECORDSET = $row['total'];
  
   $query="select * from epr_department limit  $page, $MAX";
...
function deleteNature($fid,$db)
 {	
	 global $PROMPT;
  $nature = implode("," ,$fid);
 	$delete = "delete from epr_department where nature_id in ($nature)";
	$db->query($delete);
