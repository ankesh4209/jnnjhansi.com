...
function viewcomplainant($db,$db1,$sdate,$edate,$category,$status)
 {
   ...
   global   $cid ,$cname,$caddress,$ccontact,$ccno,$cdetails,$status,$nature_name,$complain_list,$officer_text,$sdate_text,$edate_text,$c_mode;
   ...
   else
    {
	 ...
	 $query="select * from complainant where department='$category' and c_date>='$sdate' $edate1 $status1";
   $db->query($query);
		if($db->num_rows())
		{
			$slno=$page+1;
		   ...
		   $complain_list.="<tr>
                           <td align='left' align='left' style='padding-left:5px;' width='10%'>$slno</td>
                           ...
                           <td align='left' width='10%'><span style='font-family:Kruti Dev 011;font-size:14px;'>$medium</span></td>
