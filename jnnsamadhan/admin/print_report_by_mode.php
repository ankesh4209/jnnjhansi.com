...
$PAGE_CONTENTS	= ReadTemplate("../$TEMPLATE_DIR/admin/print_report_by_mode.html");
...
function viewcomplainant($db,$db1,$sdate,$edate,$category,$status,$cmode,$corporator,$wardno)
 {
  ...
  if($edate!="")
   ...
   $query="select * from complainant where 1=1 $status1 $sdate1 $edate1 $category1 $cmode $corporator $wardno";
...
function viewAllcomplainant($db,$db1,$sdate,$edate,$category,$status,$cmode,$corporator,$wardno)
 {
  ...
  if($edate!="")
   ...
   $date_diff=$date_diff/(24*60*60);
	  ...
	  $query="select * from complainant where 1=1 $status1 $sdate1 $edate1 $c_mode";
