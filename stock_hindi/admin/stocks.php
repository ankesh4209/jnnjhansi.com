   ...
   $S1	= $S2 = ReadTemplate("../$TEMPLATE_DIR/admin/stocksDisplayGrid.html");
	...
	$MAX=10;
	 $lastrow=$MAX+$page;
	  ...
	  $count="SELECT count(t.total) as total FROM ( SELECT count(1) as total from `epr_stocks`group by InvoiceNo) as t";
	 
   $db->query($count);
	 $row = $db->fetch_assoc();
   $TOTAL_RECORDSET = $row['total'];
  
   $query="select * from epr_stocks GROUP BY InvoiceNo order by StockId DESC limit  $page, $MAX ";
		...
		if($db->num_rows())
		{
		  $slno=$page+1;
			while($rows = $db->fetch_array())
			{
			  ...
			  $strPrice='';
			  $STotal=0;
              $query1="select * from epr_stocks where InvoiceNo='".$rows['InvoiceNo']."'";
