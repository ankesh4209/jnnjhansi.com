				...
				for($i=0;$i<count($ItemType);$i++)
				{
					 ...
					 if($ItemType[$i]!="" && $ItemId[$i]!=""  && $Qty[$i]!=""  && $Price[$i]!="")
					  { 
						...
						//$Price=$Price[$i];
						//echo 'dsdsds';
						$insert="insert into epr_stocks(ItemType,ItemId,PurchasingDate,InvoiceNo,ChequeNo,Qty,Price) values('".$ItemType[$i]."','".$ItemId[$i]."','$PurchasingDate','$InvoiceNo','$ChequeNo','".$Qty[$i]."','".$Price[$i]."')";
...
function GetCategory($db)
 {
    ...
    $row=$db->query($sql);
      $category.="<select  name='ItemType[]'  style='font-family: kruti_dev_010regular;font-size:15px;' onchange='ShowItems(this.value)'>";
 ...
 {
	global $edit_items;
	$sql="select * from epr_items order by ItemId";
    $row=$db->query($sql);
    if($db->num_rows())
    {
	  ...
	  while($res=$db->fetch_array($row))
       {
         ...
         $ItemName = $res['ItemName'];
         	$edit_items.="<option value='$item_id' style='font-family: kruti_dev_010regular;font-size:15px;'>$ItemName</option>";
