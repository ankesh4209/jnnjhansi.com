  ...
  if($edate!="")
  ...
  else
   {
   ...
   else
    {
	 ...
	 $count="select count(c_id) as total from complainant where status='0' $sdate1 $edate1 $category1 $search_key ";
   $db->query($count);
	 $row = $db->fetch_assoc();
   $TOTAL_RECORDSET = $row['total'];
  
   $query="select * from complainant where status='0' $search_key $sdate1 $edate1 $category1  order by c_id desc limit  $page, $MAX";
