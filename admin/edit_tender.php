...
function editpdffile($db,$pdfId)
{
   ...
   $tdate=$tdate['2']."-".$tdate['1']."-".$tdate['0']; 
	  ...
	  if(is_uploaded_file($_FILES['pdf_file']['tmp_name'],$uploadPath)){
	  ...
	  if(move_uploaded_file ($_FILES['pdf_file']['tmp_name'],$uploadPath))
	   {
			...
			$pdfname=$_FILES['pdf_file']['name'];
		     ...
		     $query="update pdffiles set Pdf_Name='$pdfname',Pdf_Desc ='$p_description',AddedDate='$tdate',status='$status' where Pdf_Id='$pdfId'";
