...
fo”ks’k
 ...
 <!-- <span class="cancelledrowindicator" style="display:inline-block;">&nbsp;</span>&nbsp;<span style="font-family: kruti_dev_010regular;font-size:15PX;">dsfUly dk;Z </span> -->
   ...
   if($PhysicalProgress=='All'){
	   /*$Tquery="select count(*) as  Total from cw_progress cw_prg inner join cw_estimation cw_e on cw_prg.EstmtId=cw_e.EstmtId INNER JOIN cw_tenders cw_t ON cw_t.EstmtId=cw_prg.EstmtId where cw_t.Status='Won' and cw_prg.Status!='Cancelled' and 1 $strSql";
   ...
   $S1	= $S2 = ReadTemplate("$TEMPLATE_DIR/contractor_wiseDisplayGrid.html");
	...
	$count="select count(*) as total from cw_progress cw_prg INNER JOIN cw_estimation cw_e ON cw_prg.EstmtId=cw_e.EstmtId INNER JOIN cw_tenders cw_t ON cw_prg.EstmtId=cw_t.EstmtId where cw_t.Status='Won' and cw_prg.Status!='Cancelled' $strSql";
	 
    $db->query($count);
    $row = $db->fetch_assoc();
    $TOTAL_RECORDSET = $row['total'];
  
    $query="select cw_prg.* from cw_progress cw_prg INNER JOIN cw_estimation cw_e ON cw_prg.EstmtId=cw_e.EstmtId INNER JOIN cw_tenders cw_t ON cw_prg.EstmtId=cw_t.EstmtId where cw_t.Status='Won' and cw_prg.Status!='Cancelled' $strSql order by cw_e.WardNo ASC limit  $page, $MAX ";
