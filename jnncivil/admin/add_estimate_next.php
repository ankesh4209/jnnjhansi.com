...
if(isset($_POST['submit']) && $_POST['submit']=='Next')
{
 ...
 if($_SERVER['SERVER_NAME']=='localhost')
  {
	 $uploadPath=$DOCUMENT_ROOT.'jnncivil/media/'.$_FILES['image']['name'];
  }
  else
  {
	  $uploadPath=$DOCUMENT_ROOT.'/jnncivil/media/'.$_FILES['image']['name'];
	...
	$CEEmployeeCode=$_POST['EXnEmployeeCode'];
	$AddedDate=$_POST['AddedDate'];
	$rateids=$_POST['rid'];
	$estimateamt=$_POST['estimateamt'];
	...
	$CEInfo=GetOfficerInfo($db,$CEEmployeeCode);
	GetItemsAmont($db,$rateids);

}


if(isset($_POST['submit']) && $_POST['submit']=='Submit Estimate'){

     $result=addEstimate($db);
       if($result)
	  {
	   echo "<script type='text/javascript'>
        <!-- 
         window.location = 'estimates.php?msg=succ'
        //-->
        </script>";   
	  }
...
$PAGE_CONTENTS	= ReadTemplate("../$TEMPLATE_DIR/admin/add_estimate_next.html");
...
function addEstimate($db)
 {
		   ...
		   $insert="insert into cw_estimation(MadId,MadAmtId,WorkName,ItemQtys,TotalAmount,JEEmployeeCode,AEEmployeeCode,EXnEmployeeCode,WardNo,AreaName,AddedDate,ProposerName,ProposedWorkDate,file) values('".$MadId."','".$MadAmtId."','".$WorkName."','".$ItemQtys."','".$TotalAmount."','".$JEEmployeeCode."','".$AEEmployeeCode."','".$EXnEmployeeCode."','".$WardNo."','".$AreaName."','".$AddedDate."','".$ProposerName."','".$ProposedDate."' ,'".$Image."')";
		   $db->query($insert);
