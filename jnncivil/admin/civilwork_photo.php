...
for($k=1;$k<=60;$k++){
   ...
   if($k>32){
     $WardList2.="<a href='civilwork_photo.php?wardno=$k'>".$k."</a>&nbsp;";
   }else{
     $WardList1.="<a href='civilwork_photo.php?wardno=$k'>".$k."</a>&nbsp;";
   }
}
...
{
	$PROMPT='Civil work photo has been added successfullly.';
}
if(isset($_GET['msg']) && $_GET['msg']=='e_succ')
{
	$PROMPT='Civil work photo has been edited successfullly.';
}
viewpage($db,$db1);

 
$PAGE_CONTENTS	= ReadTemplate("../$TEMPLATE_DIR/admin/civilwork_photo.html");
...
function viewpage($db,$db1)
 {
      ...
      global $TOTAL_RECORDSET,$PAGE_NAVS,$Status,$MadId,$MadName,$TotalAmount,$WorkName,$WardNo,$AreaName,$AddedDate,$edit ,  $completedphoto,  $startphoto, $inprogressphoto ;
     $S1	= $S2 = ReadTemplate("../$TEMPLATE_DIR/admin/civilwork_photoGrid.html"); 
	 $CURRENT_PAGE_NO=0;
	 $TOTAL_PAGES=0;
	 $TOTAL_RECORDSET=0;	 
	 $page = $_GET['page'];	 
	 ...
	 $count="select count(id) as total from cw_photo where $sql"; 
     $db->query($count);
	 $row = $db->fetch_assoc();
   $TOTAL_RECORDSET = $row['total'];
  
   $query="select * from cw_photo where $sql order by id DESC ,wardno ASC limit  $page, $MAX ";
   ...
   $db->query($query);
		if($db->num_rows())
		{
		  $slno=$page+1;
            ...
            //  $AreaName=str_replace('\\','',stripslashes($rows['AreaName']));
			  ...
			  if($Status=='1'){
			    ...
			    //$Status='<a href="edit_estimate.php?cid='.$cid.'">Edit</a>/';
				$Status="Completed";
			  }
		 ...
		 if($page > 0)
			{	$prevpage=$page - $MAX;
				$PREV_PAGE_LINK="<<a href='civilwork_photo.php?page=$prevpage&max=$MAX&$next_links&wardno=".$_GET['wardno']."' >Prev</a>";
			}
			
			if($TOTAL_RECORDSET > $lastrow)
			{	$NEXT_PAGE_LINK="<a href='civilwork_photo.php?page=$lastrow&max=$MAX&$next_links&wardno=".$_GET['wardno']."' >Next></a>";
			}
       ...
       if ($lastrow-$i==$MAX)
          ...
          $PAGE_NAVS.=" <B>".$toPrint."</b> | ";
				...
				{	
          $PAGE_NAVS.=" <a href='civilwork_photo.php?page=$i&max=$MAX&left_id=1&$next_links&wardno=".$_GET['wardno']."' >$toPrint</a> |";
				}
				$TOTAL_PAGES=$toPrint;
			}
...
function deleteaboutus($cid, $db)
 {	
	global $PROMPT;
   	...
   	$product = implode(",", $cid);
    $sel_reg_id="delete from cw_photo where id in ($product)"; 
    $db->query($sel_reg_id);
