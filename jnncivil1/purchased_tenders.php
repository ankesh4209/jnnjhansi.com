...
if($_GET['msg']=='succ'){
   $PROMPT='Tender has been purchased successfullly.';
}
...
$PAGE_CONTENTS	= ReadTemplate("$TEMPLATE_DIR/purchased_tenders.html");
...
function viewTenders($db,$db1)
 {
   ...
   global $TOTAL_RECORDSET,$PAGE_NAVS,$Status,$TotalCnt,$TotalAmount,$WorkName,$WardNo,$TenderStartDate,$TenderEndDate,$BiddingStartDate,$BiddingEndDate,$TenderCategory,$PurchaseAmount,$action;
   
   $S1	= $S2 = ReadTemplate("$TEMPLATE_DIR/purchasedtendersDisplayGrid.html");
	...
	$MAX=10;
	 ...
	 $cur_time=mktime(date('H'),date('i'),date('s'),date('m'),date('d'),date('Y'))+19800;
	  $count="select count(TenderId) as total from cw_tender_purchased where   ContractorId=".$_SESSION['ContractorId']." order by PurchaseDate DESC";
	 
    $db->query($count);
	 $row = $db->fetch_assoc();
   $TOTAL_RECORDSET = $row['total'];
  
    $query="select * from cw_tender_purchased where   ContractorId=".$_SESSION['ContractorId']." order by PurchaseDate DESC limit  $page, $MAX ";
...
//print_r($row1);
			  ...
			  //echo"<br>".$Biddin=date('d/m/Y h:i:s A',(mktime(date('H'),date('i'),date('s'),date('m'),date('d'),date('Y')))+19800);05:30hr
			  ...
			  $purchase_sql="select count(*) as cnt from cw_tender_bidded where ContractorId=".$_SESSION['ContractorId']." and TenderId=".$cid;
			  ...
			  if($cnt>0){
			     $action='Bidded';
				  ...
				  if($row1['BiddingStart']<$cur_time){
					$action="<a href='bidding.php?cid=$cid'>Bid</a>";
				  }else{
					  $action='Bid';
				  }
			  }
