...
$PAGE_CONTENTS	= ReadTemplate("../$TEMPLATE_DIR/admin/bidded_tenders.html");
...
function viewpage($db,$db1)
 {
   ...
   global $TOTAL_RECORDSET,$PAGE_NAVS,$Status,$TotalCnt,$TotalAmount,$WorkName,$WardNo,$TenderStartDate,$TenderEndDate,$BiddingStartDate,$BiddingEndDate,$TenderCategory,$PurchaseAmount,$action,$BiddingCnt;
   
   $S1	= $S2 = ReadTemplate("../$TEMPLATE_DIR/admin/biddedtendersDisplayGrid.html");
	...
	$count="select count(TenderBidId) as total from cw_tender_bidded where Status='Final' group by TenderId";
	 
    $db->query($count);
	$row = $db->fetch_assoc();
    $TOTAL_RECORDSET = $row['total'];
  
    $query="select * from cw_tender_bidded where Status='Final' group by TenderId order by TenderBidId DESC limit  $page, $MAX ";
...
//echo '<pre>';
//print_r($row1);
			  $sql2="SELECT count(*) as BiddingCnt FROM cw_tender_bidded Where TenderId='".$rows['TenderId']."' and Status='Final'";
              $res2=$db1->query($sql2);
			  $row2 = $db1->fetch_array();
              $BiddingCnt=$row2['BiddingCnt'];
			  
			  $cid=$rows['TenderId'];
			  $WardNo=$row1['WardNo'];
              $WorkName=$row1['WorkName'];
              $TotalAmount=number_format($row1['TotalAmount'],2);
             
			  $TenderCategory=$row1['TenderCategory'];
			  $PurchaseAmount=$row1['PurchaseAmount'];
			  
			  $TenderStartDate=date('d/m/Y h A',$row1['TenderStart']);
			  
			  $TenderEndDate=date('d/m/Y h A',$row1['TenderEnd']);
			  ...
			  //echo $row1['BiddingEnd']."<$cur_time";
			  if($row1['BiddingEnd']<$cur_time){
			     if($BiddingCnt>=3){
					$action="<a href='biddings.php?cid=$cid'>View</a>";
				 }else{
					 $action="<a href='cancel_tender.php?cid=$cid'>Cancel</a>";
				 }
			  }else{
				  $action="--";
			  }
