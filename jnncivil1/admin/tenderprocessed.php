...
{
	$PROMPT='Tender has been assigned successfullly.';
}
...
function viewpage($db,$db1)
 {
   ...
   global $TOTAL_RECORDSET,$PAGE_NAVS,$Status,$TotalCnt,$MinRate,$TotalAmount,$WorkName,$WardNo,$ContractorName,$AddedDate,$EstimatedAmount,$DepositeType,$DepositeDetails,$WorkEndDate,$WorkStartDate,$WorkOrderNo,$FinalComments;
   ...
   $S1	= $S2 = ReadTemplate("../$TEMPLATE_DIR/admin/tenderprocessedDisplayGrid.html");
	...
	$MAX=10;
	 $lastrow=$MAX+$page;
	  ...
	  $count="select * from cw_tenders where Status='Won'";
   ...
   //$TOTAL_RECORDSET = $rowtotal'total'];
    ...
    $query="select * from cw_tenders  where Status='Won' order by TenderId DESC limit  $page, $MAX ";
		...
		if($db->num_rows())
		{
		  $slno=$page+1;
			while($rows = $db->fetch_array())
			{
              ...
              $sql="SELECT * FROM cw_tender_bidded where TenderId='".$rows['TenderId']."' and TenderStatus='Won'";
              $res1=$db1->query($sql);
			  ...
			  $FinalComments=$rows['FinalComments'];

              $WorkStartDate=date('d/m/Y',strtotime($rows['WorkStartDate']));
              $WorkEndDate=date('d/m/Y',strtotime($rows['WorkEndDate']));
              ...
              $DepositeType=$row1['DepositeType'];
              $DepositeDetails=$row1['DepositeDetails'];
              $MinRate=$row1['BiddingRate'];
              $TotalAmount=$EstimatedAmount-($EstimatedAmount*$row1['BiddingRate'])/100;
