...
{
	$PROMPT='Amount has been added successfullly.';
}
if(isset($_GET['msg']) && $_GET['msg']=='e_succ')
{
	$PROMPT='Amount has been edited successfullly.';
}
viewpage($db,$db1,$rid);
...
function viewpage($db,$db1,$rid)
 {
   ...
   global $TOTAL_RECORDSET,$PAGE_NAVS,$PayId,$WorkOrderNo,$Installment,$PayDate,$Amount,$CheckNo,$CheckDate;
   ...
   $S1	= $S2 = ReadTemplate("../$TEMPLATE_DIR/admin/paymentDisplayGrid.html");
	...
	$MAX=10;
	 $lastrow=$MAX+$page;
	  ...
	  $count="select count(PayId) as total from cw_payments";
	 
   $db->query($count);
	 $row = $db->fetch_assoc();
   $TOTAL_RECORDSET = $row['total'];
  
   $query="select * from cw_payments order by WorkOrderNo,PayId DESC limit  $page, $MAX ";
   ...
   $db->query($query);
		if($db->num_rows())
		{
		 ...
		 if($page > 0)
			{	$prevpage=$page - $MAX;
				$PREV_PAGE_LINK="<<a href='payments.php?page=$prevpage&max=$MAX&$next_links' >Prev</a>";
			}
			
			if($TOTAL_RECORDSET > $lastrow)
			{	$NEXT_PAGE_LINK="<a href='payments.php?page=$lastrow&max=$MAX&$next_links' >Next></a>";
			}
       ...
       if ($lastrow-$i==$MAX)
          ...
          $PAGE_NAVS.=" <B>".$toPrint."</b> | ";
				...
				{	
          $PAGE_NAVS.=" <a href='payments.php?page=$i&max=$MAX&left_id=1&$next_links' >$toPrint</a> |";
				}
				$TOTAL_PAGES=$toPrint;
			}
