...
function viewpage($db,$db1)
 {
   ...
   global $TOTAL_RECORDSET,$PAGE_NAVS,$Status,$MadId,$MadName,$TotalAmount,$WorkName,$WardNo,$AreaName,$AddedDate;
   ...
   $S1	= $S2 = ReadTemplate("../$TEMPLATE_DIR/admin/estmtDisplayGrid.html");
	...
	$MAX=10;
	 $lastrow=$MAX+$page;
	  ...
	  $count="select count(EstmtId) as total from cw_estimation";
	 
   $db->query($count);
	 $row = $db->fetch_assoc();
   $TOTAL_RECORDSET = $row['total'];
  
   $query="select * from cw_estimation order by EstmtId DESC limit  $page, $MAX ";
   ...
   $db->query($query);
		if($db->num_rows())
		{
		 ...
		 if($page > 0)
			{	$prevpage=$page - $MAX;
				$PREV_PAGE_LINK="<<a href='estimates.php?page=$prevpage&max=$MAX&$next_links' >Prev</a>";
			}
			
			if($TOTAL_RECORDSET > $lastrow)
			{	$NEXT_PAGE_LINK="<a href='estimates.php?page=$lastrow&max=$MAX&$next_links' >Next></a>";
			}
       ...
       if ($lastrow-$i==$MAX)
          ...
          $PAGE_NAVS.=" <B>".$toPrint."</b> | ";
				...
				{	
          $PAGE_NAVS.=" <a href='estimates.php?page=$i&max=$MAX&left_id=1&$next_links' >$toPrint</a> |";
				}
				$TOTAL_PAGES=$toPrint;
			}
