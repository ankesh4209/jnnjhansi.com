...
$PAGE_CONTENTS	= ReadTemplate("$TEMPLATE_DIR/approved_tenders.html");
...
function viewTenders($db,$db1)
 {
   ...
   $S1	= $S2 = ReadTemplate("$TEMPLATE_DIR/approvedtendersDisplayGrid.html");
	...
	$MAX=10;
	 ...
	 $count="select count(TenderId) as total from cw_tender_bidded where TenderStatus='Won' and ContractorId=".$_SESSION['ContractorId']." order by BiddingDate DESC";
	 
    $db->query($count);
	 $row = $db->fetch_assoc();
   $TOTAL_RECORDSET = $row['total'];
  
    $query="select * from cw_tender_bidded where  TenderStatus='Won' and ContractorId=".$_SESSION['ContractorId']." order by BiddingDate DESC limit  $page, $MAX ";
