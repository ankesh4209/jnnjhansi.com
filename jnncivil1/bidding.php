...
if($_POST["Save"]=='Save Bid')
{
		...
		if($result){
			...
			$PROMPT='Bidding could not be saved';
		}
	viewpage($db,$db1,$cid);
...
$PAGE_CONTENTS	= ReadTemplate("$TEMPLATE_DIR/bidding.html");
...
function viewpage($db,$db1,$cid)
 {
   global $cid,$status,$MadId,$MadName,$TotalAmount,$WorkName,$WardNo,$AreaName,$AddedDate,$Status,$Approved,$Cancelled,$Pending,$JEInfo,$AEInfo,$EXnInfo,$ProposerName,$TenderStartDate,$TenderEndDate,$BiddingStartDate,$BiddingEndDate,$TenderCategory,$PurchaseAmount,$Comments;
...
function SaveBid( $db,$cid)
{
   ...
   global $PROMPT;
	...
	$insert="insert into cw_tender_bidded(TenderId,ContractorId,BiddingRate,BidType,BiddingAmount,BiddingDate,DepositeType,DepositeDetails,Status) values('".$cid."','".$_SESSION['ContractorId']."','".$BiddingRate."','".$BidType."','".$BiddingAmount."','".date('Y-m-d h:i:s')."','".$DepositeType."','".$DepositeDetails."','Pending')";
    $db->query($insert);	 
