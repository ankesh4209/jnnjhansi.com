...
if($_POST["submit"] && $_POST["submit"]=='Purchase Tender')
{	
 		$result=TenderProcessing( $db,$cid);
		if($result){
			header ("Location: purchased_tenders.php?msg=succ"); 				
			
		}else{
			$PROMPT='Tender purchasing could not be processed';
		}
	viewpage($db,$db1,$cid);
...
$PAGE_CONTENTS	= ReadTemplate("$TEMPLATE_DIR/tender_details.html");
...
function viewpage($db,$db1,$cid)
 {
   global $cid,$status,$MadId,$MadName,$TotalAmount,$WorkName,$WardNo,$AreaName,$AddedDate,$Status,$Approved,$Cancelled,$Pending,$JEInfo,$AEInfo,$EXnInfo,$ProposerName,$TenderStartDate,$TenderEndDate,$BiddingStartDate,$BiddingEndDate,$TenderCategory,$PurchaseAmount,$Comments,$Vat,$TotalPurchaseAmt;
   ...
   $Vat=$tender_rows['Vat'];
   $TotalPurchaseAmt=$PurchaseAmount+($PurchaseAmount*$Vat)/100;
   $Comments=nl2br($tender_rows['Comments']);
...
function TenderProcessing( $db,$cid)
{
   ...
   global $PROMPT;
	...
	$insert="insert into cw_tender_purchased(TenderId,ContractorId,PurchaseAmount,PurchaseDate) values('".$cid."','".$_SESSION['ContractorId']."','".$TotalPurchaseAmt."','".date('Y-m-d h:i:s')."')";
    $db->query($insert);	 
