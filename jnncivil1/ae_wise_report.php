...
if($_GET['AE']=='Single'){
	$radioAE=$_GET['AE'];
	$radioSingle='checked';
	$AEEmployeeCode=$_GET['AEEmployeeCode'];
    GetAE($db,$db1,$AEEmployeeCode);
}else{
	$radioAE=$_GET['AE'];
	$radioAll='checked';
}
...
if(ISSET($_GET['showreport'])){
	viewreports($db,$db1,$radioAE,$radioSingle,$radioAll,$AEEmployeeCode,$PhysicalProgress,$Periods,$FromDate,$ToDate);
}
 
$PAGE_CONTENTS	= ReadTemplate("$TEMPLATE_DIR/ae_wise_report.html");
...
function viewreports($db,$db1,$radioAE,$radioSingle,$radioAll,$AEEmployeeCode,$PhysicalProgress,$Periods,$FromDate,$ToDate)
 {
   ...
   global $TOTAL_RECORDSET,$PAGE_NAVS,$Status,$wardno,$workname,$WorkOrderNo,$estimateamt,$ContractorName,$appamt,$appdate,$workstartdate,$workenddate,$Payments,$CheckNos,$Comment,$MadName,$trclass,$AEWork;
...
fo”ks’k
 ...
 <span class="cancelledrowindicator" style="display:inline-block;">&nbsp;</span>&nbsp;<span style="font-family: kruti_dev_010regular;font-size:15PX;">dsfUly dk;Z </span>
   ...
   $strSql='';
   if($AEEmployeeCode!=''){
	   $strSql.=" and cw_e.AEEmployeeCode='$AEEmployeeCode'";
	   $strSql2.=" and cw_e.AEEmployeeCode='$AEEmployeeCode'";
   ...
   if($AEEmployeeCode!=''){
	$emp_sql="SELECT EmployeeName FROM employeeinfo WHERE EmployeeCode='$AEEmployeeCode'";
	$emp_res=$db->query($emp_sql);
	$emp_rows=$db->fetch_array($emp_res);
	$AEName=$emp_rows['EmployeeName'];
   $AEInfo="<span style='padding-right:10px;font-family: kruti_dev_010regular;font-size:15px;'>$AEName ds</span>";
	   
   }else{
   $x='ds';
   }
   $AEWork="<div class='pis_list'><table width='1010' border='1' cellspacing='0' cellpadding='5' align='center' ><tbody>
	  <tr style='background:#000;color:#fff;font-weight:bold;'><td colspan='7'><span style='padding-right:10px;font-family: kruti_dev_010regular;font-size:15px;'>Lkgk;d vfHk;Urk $AEInfo $x dk;ksZa dk lkjka”k</span></td></tr>
   ...
   $S1	= $S2 = ReadTemplate("$TEMPLATE_DIR/ae_wiseDisplayGrid.html");
    ...
    $db->query($query);
		if($db->num_rows())
		{
		 ...
		 if($page > 0)
			{	$prevpage=$page - $MAX;
				$PREV_PAGE_LINK="<<a href='ae_wise_report.php?page=$prevpage&max=$MAX&left_id=1&AE=$radioExn&AEEmployeeCode=$AEEmployeeCode&PhysicalProgress=$PhysicalProgress&Periods=$Periods&FromDate=$FromDate&ToDate=$ToDate&showreport=$showreport' >Prev</a>";
			}
			
			if($TOTAL_RECORDSET > $lastrow)
			{	$NEXT_PAGE_LINK="<a href='ae_wise_report.php?page=$lastrow&max=$MAX&left_id=1&AE=$radioExn&AEEmployeeCode=$AEEmployeeCode&PhysicalProgress=$PhysicalProgress&Periods=$Periods&FromDate=$FromDate&ToDate=$ToDate&showreport=$showreport' >Next></a>";
			}
       ...
       if ($lastrow-$i==$MAX)
          ...
          $PAGE_NAVS.=" <B>".$toPrint."</b> | ";
				...
				{	
          $PAGE_NAVS.=" <a href='ae_wise_report.php?page=$i&max=$MAX&left_id=1&AE=$radioExn&AEEmployeeCode=$AEEmployeeCode&PhysicalProgress=$PhysicalProgress&Periods=$Periods&FromDate=$FromDate&ToDate=$ToDate&showreport=$showreport' >$toPrint</a> |";
				}
				$TOTAL_PAGES=$toPrint;
			}
...
function GetAE($db,$db1,$EmpId='')
 {	
	global $AEofficers;
	$query="select distinct(AEEmployeeCode) from cw_estimation where AEEmployeeCode!=''";
	$row1=$db1->query($query);
	$arrAE=array();
	while($res1=$db1->fetch_array($row1))
     {
		$arrAE[]=$res1['AEEmployeeCode'];
	 }
    $strAE=implode(',',$arrAE);
	$sql="select EmployeeCode,EmployeeName from employeeinfo where EmployeeCode in ($strAE) order by EmployeeCode ASC";
	  ...
	  while($res=$db->fetch_array($row))
       {
			...
			if($EmployeeCode==$EmpId){
				$AEofficers.="<option value='$EmployeeCode' selected>$EmployeeName dksM $EmployeeCode </option>";
			}else{
				$AEofficers.="<option value='$EmployeeCode' >$EmployeeName dksM $EmployeeCode </option>";
			}
