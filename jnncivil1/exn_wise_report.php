...
if($_GET['Exn']=='Single'){
	$radioExn=$_GET['Exn'];
	$radioSingle='checked';
	$EXnEmployeeCode=$_GET['EXnEmployeeCode'];
    GetExn($db,$db1,$EXnEmployeeCode);
}else{
	$radioExn=$_GET['Exn'];
	$radioAll='checked';
}
...
if(ISSET($_GET['showreport'])){
	viewreports($db,$db1,$radioExn,$radioSingle,$radioAll,$EXnEmployeeCode,$PhysicalProgress,$Periods,$FromDate,$ToDate);
}
 
$PAGE_CONTENTS	= ReadTemplate("$TEMPLATE_DIR/exn_wise_report.html");
...
function viewreports($db,$db1,$radioExn,$radioSingle,$radioAll,$EXnEmployeeCode,$PhysicalProgress,$Periods,$FromDate,$ToDate)
 {
   ...
   global $TOTAL_RECORDSET,$PAGE_NAVS,$Status,$wardno,$workname,$WorkOrderNo,$estimateamt,$ContractorName,$appamt,$appdate,$workstartdate,$workenddate,$Payments,$CheckNos,$Comment,$MadName,$trclass,$EXnWork;
...
fo”ks’k
 ...
 <span class="cancelledrowindicator" style="display:inline-block;">&nbsp;</span>&nbsp;<span style="font-family: kruti_dev_010regular;font-size:15PX;">dsfUly dk;Z </span>
   ...
   $strSql='';
   if($EXnEmployeeCode!=''){
	   $strSql.=" and cw_e.EXnEmployeeCode='$EXnEmployeeCode'";
	   $strSql2.=" and cw_e.EXnEmployeeCode='$EXnEmployeeCode'";
   ...
   if($PhysicalProgress=='All'){
	   $Tquery="select count(*) as  Total from cw_progress cw_prg inner join cw_estimation cw_e on cw_prg.EstmtId=cw_e.EstmtId where 1 $strSql";
   ...
   if($EXnEmployeeCode!=''){
   $emp_sql="SELECT EmployeeName FROM employeeinfo WHERE EmployeeCode='$EXnEmployeeCode'";
   $emp_res=$db->query($emp_sql);
   $emp_rows=$db->fetch_array($emp_res);
   $ExnName=$emp_rows['EmployeeName'];
	$ExnInfo="<span style='padding-right:10px;font-family: kruti_dev_010regular;font-size:15px;'>$ExnName ds</span>";
	   
   }else{
   $x='ds';
   }

   $EXnWork="<div class='pis_list'><table width='1010' border='1' cellspacing='0' cellpadding='5' align='center' ><tbody>
	  <tr style='background:#000;color:#fff;font-weight:bold;'><td colspan='7'><span style='padding-right:10px;font-family: kruti_dev_010regular;font-size:15px;'>vf/k'kklh vfHk;Urk $ExnInfo $x dk;ksZa dk lkjka”k</span></td></tr>
   ...
   $S1	= $S2 = ReadTemplate("$TEMPLATE_DIR/exn_wiseDisplayGrid.html");
	...
	$count="select count(ProgId) as total from cw_progress cw_prg INNER JOIN cw_estimation cw_e ON cw_prg.EstmtId=cw_e.EstmtId $strSql";
	 
    $db->query($count);
    $row = $db->fetch_assoc();
    $TOTAL_RECORDSET = $row['total'];
  
    $query="select cw_prg.* from cw_progress cw_prg INNER JOIN cw_estimation cw_e ON cw_prg.EstmtId=cw_e.EstmtId $strSql order by WardNo ASC limit  $page, $MAX ";
    ...
    $db->query($query);
		if($db->num_rows())
		{
		 ...
		 if($page > 0)
			{	$prevpage=$page - $MAX;
				$PREV_PAGE_LINK="<<a href='exn_wise_report.php?page=$prevpage&max=$MAX&left_id=1&Exn=$radioExn&EXnEmployeeCode=$EXnEmployeeCode&PhysicalProgress=$PhysicalProgress&Periods=$Periods&FromDate=$FromDate&ToDate=$ToDate&showreport=$showreport' >Prev</a>";
			}
			
			if($TOTAL_RECORDSET > $lastrow)
			{	$NEXT_PAGE_LINK="<a href='exn_wise_report.php?page=$lastrow&max=$MAX&left_id=1&Exn=$radioExn&EXnEmployeeCode=$EXnEmployeeCode&PhysicalProgress=$PhysicalProgress&Periods=$Periods&FromDate=$FromDate&ToDate=$ToDate&showreport=$showreport' >Next></a>";
			}
       ...
       if ($lastrow-$i==$MAX)
          ...
          $PAGE_NAVS.=" <B>".$toPrint."</b> | ";
				...
				{	
          $PAGE_NAVS.=" <a href='exn_wise_report.php?page=$i&max=$MAX&left_id=1&Exn=$radioExn&EXnEmployeeCode=$EXnEmployeeCode&PhysicalProgress=$PhysicalProgress&Periods=$Periods&FromDate=$FromDate&ToDate=$ToDate&showreport=$showreport' >$toPrint</a> |";
				}
				$TOTAL_PAGES=$toPrint;
			}
...
function GetExn($db,$db1,$EmpId='')
 {	
	global $Exnofficers;
	$query="select distinct(EXnEmployeeCode) from cw_estimation where EXnEmployeeCode!=''";
	$row1=$db1->query($query);
	$arrEXn=array();
	while($res1=$db1->fetch_array($row1))
     {
		$arrEXn[]=$res1['EXnEmployeeCode'];
	 }
    $strEXn=implode(',',$arrEXn);
	$sql="select EmployeeCode,EmployeeName from employeeinfo where EmployeeCode in ($strEXn) order by EmployeeCode ASC";
	  ...
	  while($res=$db->fetch_array($row))
       {
			...
			if($EmployeeCode==$EmpId){
				$Exnofficers.="<option value='$EmployeeCode' selected>$EmployeeName dksM $EmployeeCode </option>";
			}else{
				$Exnofficers.="<option value='$EmployeeCode' >$EmployeeName dksM $EmployeeCode </option>";
			}
