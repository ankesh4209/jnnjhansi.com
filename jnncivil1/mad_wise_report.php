...
if(ISSET($_GET['showreport'])){
	...
	viewreports($db,$db1,$radioMad,$radioSingle,$radioAll,$MadId,$PhysicalProgress,$Periods,$FromDate,$ToDate,$showreport);
}
 
$PAGE_CONTENTS	= ReadTemplate("$TEMPLATE_DIR/mad_wise_report.html");
...
function viewreports($db,$db1,$radioMad,$radioSingle,$radioAll,$MadId,$PhysicalProgress,$Periods,$FromDate,$ToDate,$showreport)
 {
   ...
   global $TOTAL_RECORDSET,$PAGE_NAVS,$Status,$wardno,$workname,$WorkOrderNo,$estimateamt,$ContractorName,$appamt,$appdate,$workstartdate,$workenddate,$Payments,$CheckNos,$Comment,$MadName,$trclass,$MadWorkSummary;
...
fo”ks’k
 ...
 <span class="cancelledrowindicator" style="display:inline-block;">&nbsp;</span>&nbsp;<span style="font-family: kruti_dev_010regular;font-size:15PX;">dsfUly dk;Z </span>
   ...
   $strSql='';
   if($MadId!=''){
	   $strSql.=" and cw_e.MadId='$MadId'";
	   $strSql2.=" and cw_e.MadId='$MadId'";
   ...
   if($MadId!=''){
	   $mad_sql="SELECT MadName FROM cw_mads WHERE MadId='$MadId'";
	   $mad_res=$db->query($mad_sql);
	   $mad_rows=$db->fetch_array($mad_res);
	   $MadName=$mad_rows['MadName'];
	   $MadInfo="<span style='padding-right:10px;font-family: kruti_dev_010regular;font-size:15px;'>$MadName ds</span>";
	   
   }else{
   $x='ds';
   }

   $MadWorkSummary="<div class='pis_list'><table width='1010' border='1' cellspacing='0' cellpadding='5' align='center' ><tbody>
	  <tr style='background:#000;color:#fff;font-weight:bold;font-family: kruti_dev_010regular;font-size:15px;'><td colspan='7'>Ekn $x $MadInfo dk;ksZa dk lkjka”k</td></tr>
   ...
   $S1	= $S2 = ReadTemplate("$TEMPLATE_DIR/mad_wiseDisplayGrid.html");
    ...
    $db->query($query);
		if($db->num_rows())
		{
		 ...
		 if($page > 0)
			{	$prevpage=$page - $MAX;
				$PREV_PAGE_LINK="<<a href='mad_wise_report.php?page=$prevpage&max=$MAX&Mad=$radioMad&MadId=$MadId&PhysicalProgress=$PhysicalProgress&Periods=$Periods&FromDate=$FromDate&ToDate=$ToDate&showreport=$showreport'>Prev</a>";
			}
			
			if($TOTAL_RECORDSET > $lastrow)
			{	$NEXT_PAGE_LINK="<a href='mad_wise_report.php?page=$lastrow&max=$MAX&Mad=$radioMad&MadId=$MadId&PhysicalProgress=$PhysicalProgress&Periods=$Periods&FromDate=$FromDate&ToDate=$ToDate&showreport=$showreport' >Next></a>";
			}
       ...
       if ($lastrow-$i==$MAX)
          ...
          $PAGE_NAVS.=" <B>".$toPrint."</b> | ";
				...
				{	
          $PAGE_NAVS.=" <a href='mad_wise_report.php?page=$i&max=$MAX&Mad=$radioMad&MadId=$MadId&PhysicalProgress=$PhysicalProgress&Periods=$Periods&FromDate=$FromDate&ToDate=$ToDate&showreport=$showreport' >$toPrint</a> |";
				}
				$TOTAL_PAGES=$toPrint;
			}
...
function GetMadList($db,$db1,$pMadId='')
 {	
	global $MadList;
	$sql="select MadId,MadName from cw_mads order by MadId ASC";
    $row=$db->query($sql);
      $MadList.="<select  name='MadId'  style='font-family: kruti_dev_010regular;font-size:15px;'>";
	  ...
	  while($res=$db->fetch_array($row))
       {
			...
			if($MadId!=0){
				if($MadId==$pMadId){
					$MadList.="<option value='$MadId' selected>$MadName </option>";
				}else{
					$MadList.="<option value='$MadId' >$MadName </option>";
				}
			}
