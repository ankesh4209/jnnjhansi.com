...
$PAGE_CONTENTS	= ReadTemplate("../$TEMPLATE_DIR/admin/educations.html");
...
function viewcontents($db)
 {
   ...
   global $TOTAL_RECORDSET,$PAGE_NAVS,$status,$pid,$emp_name,$emp_code,$ecode,$ename,$Education,$Edu_Type;
   
   $S1	= $S2 = ReadTemplate("../$TEMPLATE_DIR/admin/educationDisplayGrid.html");
	...
	$sql='';  
	if($_GET['Education']!=''){
		$sql.=" and Education like '%".addslashes($_GET['Education'])."%'";
		$Edu_Type=$_GET['Education'];
	}
   ...
   $db->query($query);
		if($db->num_rows())
		{
		 ...
		 if($page > 0)
			{	$prevpage=$page - $MAX;
				$PREV_PAGE_LINK="<a href='educations.php?page=$prevpage&max=$MAX' ><< Prev</a>";
			}
			
			if($TOTAL_RECORDSET > $lastrow)
			{	$NEXT_PAGE_LINK="<a href='educations.php?page=$lastrow&max=$MAX' >Next>></a>";
			}
			...
			{	
				...
				{	
					$PAGE_NAVS.=" <a href='educations.php?page=$i&max=$MAX' >$toPrint</a>";
				}
				$TOTAL_PAGES=$toPrint;
			}
		...
		{
			$PRODUCT_LIST="<tr><td colspan='7'>No education found.</td></tr>";
        }
