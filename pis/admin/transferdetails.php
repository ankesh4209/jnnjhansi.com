...
$PAGE_CONTENTS	= ReadTemplate("../$TEMPLATE_DIR/admin/transferdetails.html");
...
function viewcontents($db)
 {
   ...
   global $TOTAL_RECORDSET,$PAGE_NAVS,$status,$pid,$emp_name,$emp_code,$ecode,$ename,$E_TransferTo,$TransferTo;
   
   $S1	= $S2 = ReadTemplate("../$TEMPLATE_DIR/admin/transferDisplayGrid.html");
	...
	$sql='';  
	if($_GET['TransferTo']!=''){
		$sql.=" and TransferTo like '%".addslashes($_GET['TransferTo'])."%'";
		$E_TransferTo=$_GET['TransferTo'];
	}
   ...
   $query="select * from employeeinfo where TransferTo!='' ".$sql." order by EmployeeCode ASC limit  $page, $MAX ";
   ...
   $db->query($query);
		if($db->num_rows())
		{
		 ...
		 if($page > 0)
			{	$prevpage=$page - $MAX;
				$PREV_PAGE_LINK="<a href='transferdetails.php?page=$prevpage&max=$MAX' ><< Prev</a>";
			}
			
			if($TOTAL_RECORDSET > $lastrow)
			{	$NEXT_PAGE_LINK="<a href='transferdetails.php?page=$lastrow&max=$MAX' >Next>></a>";
			}
			...
			{	
				...
				{	
					$PAGE_NAVS.=" <a href='transferdetails.php?page=$i&max=$MAX' >$toPrint</a>";
				}
				$TOTAL_PAGES=$toPrint;
			}
		...
		{
			$PRODUCT_LIST="<tr><td colspan='7'>No transfer detail found.</td></tr>";
        }
