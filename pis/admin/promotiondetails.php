...
$PAGE_CONTENTS	= ReadTemplate("../$TEMPLATE_DIR/admin/promotiondetails.html");
...
function viewcontents($db)
 {
   ...
   global $TOTAL_RECORDSET,$PAGE_NAVS,$status,$pid,$emp_name,$emp_code,$ecode,$ename,$E_PromotionPost,$PromotionPost;
   
   $S1	= $S2 = ReadTemplate("../$TEMPLATE_DIR/admin/promotionDisplayGrid.html");
	...
	$sql='';  
	if($_GET['PromotionPost']!=''){
		$sql.=" and PromotionPost like '%".addslashes($_GET['PromotionPost'])."%'";
		$E_PromotionPost=$_GET['PromotionPost'];
	}
   ...
   $query="select * from employeeinfo where PromotionPost!='' ".$sql." order by EmployeeCode ASC limit  $page, $MAX ";
   ...
   $db->query($query);
		if($db->num_rows())
		{
		 ...
		 if($page > 0)
			{	$prevpage=$page - $MAX;
				$PREV_PAGE_LINK="<a href='promotiondetails.php?page=$prevpage&max=$MAX' ><< Prev</a>";
			}
			
			if($TOTAL_RECORDSET > $lastrow)
			{	$NEXT_PAGE_LINK="<a href='promotiondetails.php?page=$lastrow&max=$MAX' >Next>></a>";
			}
			...
			{	
				...
				{	
					$PAGE_NAVS.=" <a href='promotiondetails.php?page=$i&max=$MAX' >$toPrint</a>";
				}
				$TOTAL_PAGES=$toPrint;
			}
		...
		{
			$PRODUCT_LIST="<tr><td colspan='7'>No promotional detail found.</td></tr>";
        }
