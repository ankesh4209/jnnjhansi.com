...
$PAGE_CONTENTS	= ReadTemplate("../$TEMPLATE_DIR/admin/servicedetails.html");
...
function viewcontents($db)
 {
   ...
   global $TOTAL_RECORDSET,$PAGE_NAVS,$status,$pid,$emp_name,$emp_code,$ecode,$ename,$JobType,$Type;
   
   $S1	= $S2 = ReadTemplate("../$TEMPLATE_DIR/admin/serviceDisplayGrid.html");
	...
	$sql='';  
	if($_GET['JobType']!=''){
		$sql.=" and JobType like '%".addslashes($_GET['JobType'])."%'";
		$Type=$_GET['JobType'];
	}
   ...
   $db->query($query);
		if($db->num_rows())
		{
		 ...
		 if($page > 0)
			{	$prevpage=$page - $MAX;
				$PREV_PAGE_LINK="<a href='servicedetails.php?page=$prevpage&max=$MAX' ><< Prev</a>";
			}
			
			if($TOTAL_RECORDSET > $lastrow)
			{	$NEXT_PAGE_LINK="<a href='servicedetails.php?page=$lastrow&max=$MAX' >Next>></a>";
			}
			...
			{	
				...
				{	
					$PAGE_NAVS.=" <a href='servicedetails.php?page=$i&max=$MAX' >$toPrint</a>";
				}
				$TOTAL_PAGES=$toPrint;
			}
		...
		{
			$PRODUCT_LIST="<tr><td colspan='7'>No service detail found.</td></tr>";
        }
